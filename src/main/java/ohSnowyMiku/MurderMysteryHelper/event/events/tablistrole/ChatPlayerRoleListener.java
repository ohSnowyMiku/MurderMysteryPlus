package ohSnowyMiku.MurderMysteryHelper.event.events.tablistrole;
...
public class ChatPlayerRoleListener {
    ...
    // 聊天触发控制开关
    private static final boolean ENABLED = MurderMysteryHelperConfig.showRolesInTabListFromChatSwitch;
    ...
    private void applyRole(String playerName, String prefix, boolean addSelf) {
        ...
        //SimpleDelayedChatUtil.scheduleMessage("§d[MurderMysteryHelper] §a已自动在TabList上标记玩家: §l§f" + playerName);

        // 如果需要，也给自己标记
        if (addSelf) {
            String self = Minecraft.getMinecraft().thePlayer.getName();
            NAMETOPREFIX.put(self, prefix);
            //SimpleDelayedChatUtil.scheduleMessage("§d[MurderMysteryHelper] §a已自动在TabList上标记玩家: §l§f" + self);
        }
    }
...
//[17:41:05] [Client thread/INFO] [STDOUT/]: [ohSnowyMiku.MurderMysteryHelper.event.events.tablistrole.ChatRoleListener:checkPlayerRole:109]: 已经匹配到正确内容
