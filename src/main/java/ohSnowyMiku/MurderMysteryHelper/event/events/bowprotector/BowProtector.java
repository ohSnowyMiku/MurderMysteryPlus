package ohSnowyMiku.MurderMysteryHelper.event.events.bowprotector;
...
import static ohSnowyMiku.MurderMysteryHelper.enums.RoleEnum.DETECTIVE;
...
public class BowProtector {
    ...
    public void onSendPacket(SendPacketEvent event) {
        ...
        if (!MurderMysteryHelperConfig.bowProtectorSwitch) return;
        ...
        if (event.packet instanceof C02PacketUseEntity) {
            ...
            if (target instanceof EntityArmorStand) {
                ...
                if (checkHeldItem()) {
                    ...
                    SimpleDelayedChatUtil.scheduleMessage("§d[MurderMysteryHelper] §a已自动保护侦探弓不被献祭(可在潜行状态强制献祭)");
                }
            }
        }
    }
